# Roteiro guiado: inventário arquitetural e funcional de aplicações

Use este núcleo com um ou mais perfis em `docs/languages/` para analisar um
codebase. Selecione apenas perfis sustentados pelos artefatos do repositório; em
um repositório multilíngue, registre o escopo de cada perfil e não atribua um
artefato à linguagem errada. Produza conclusões rastreáveis: todo fato deve
apontar para arquivo, pacote, símbolo, configuração ou teste. Não trate
convenções ou nomes como prova de comportamento.
...
2. Leia o `README`, os manifestos de dependência e build definidos pelo perfil,
   `Makefile` ou scripts equivalentes, arquivos de CI/CD, Dockerfiles,
   Helm/Kubernetes e configurações de ambiente.
3. Aplique o perfil selecionado para identificar módulos, fontes, recursos,
   artefatos de build, pontos de entrada, testes e migrations.
4. Trace os pontos de entrada para inicialização, injeção de dependências,
   handlers, jobs e encerramento da aplicação.

Entregue uma tabela inicial:

| Item | Localização | Papel observado | Confiança |
|---|---|---|---|
| Projeto/módulo | `<manifesto do perfil>` | Nome, versão e submódulo | Alta |
| Executável/comando | `<fonte ou configuração>` | Ponto de entrada | Alta |
| Configuração | `...` | Fontes e chaves lidas | Média/Alta |

## 3. Inventário de dependências, componentes e frameworks

1. Extraia dependências e versões dos manifestos definidos pelo perfil; use os
   arquivos de lock apenas para complementar versões transitivas relevantes.
2. Agrupe dependências externas por finalidade: HTTP/RPC, configuração, DI, logs,
   métricas, tracing, autenticação, persistência, mensageria, serialização e testes.
3. Liste os componentes próprios e sua responsabilidade, consumidores e
   dependências de saída.
4. Identifique frameworks por uso concreto de APIs, anotações, configuração ou
   inicialização — não apenas pela presença em um manifesto.
5. Destaque drivers SQL, ORMs/query builders, bibliotecas Kafka, clientes Redis e
   bibliotecas de serialização de mensagens quando forem usados.
...
1. Localize dependências, clientes e configuração de Kafka, incluindo wrappers
   internos e os critérios definidos pelo perfil de linguagem.
2. Identifique versão, componente que encapsula o cliente e como brokers, TLS, SASL,
...
1. Localize DSNs, variáveis de ambiente, fábrica de conexões e pool. Identifique
   o driver e a biblioteca de acesso conforme os critérios do perfil selecionado.
...
10. Localize chamadas a functions e procedures em SQL, query builders, drivers e
    ...
    componente, arquivo ou símbolo responsável e ao fluxo funcional, indicando os
...
| Recurso | Banco/tipo | Tabela/coluna, sequência ou rotina | Assinatura/operação | Origem na aplicação | Transação/erros | Fluxos | Evidência |
|---|---|---|---|---|---|---|---|
...
| ID/lote | Operação e mecanismo | Formação e origem dos dados | Tamanho/critério | Origem na aplicação | Tabelas/comandos | Transação/commit | Erro/rollback/retry | Fluxo/evidência |
|---|---|---|---|---|---|---|---|---|
...
1. Localize dependências, wrappers internos e configuração de clientes Redis.
   Registre versão, endereços ou sentinelas, banco lógico, credenciais, TLS,
   timeouts e origem de cada configuração, sem expor valores secretos.
...
Procure DDL, migrations, modelos de dados, mapeamentos, serializadores, uploads e
...
| Tipo/coluna | Tipo no banco | Representação na aplicação | Leitura/escrita | Conversão/serialização | Fluxo exposto | Riscos/evidência |
|---|---|---|---|---|---|---|
...
1. Localize dependências, wrappers internos, inicialização e configuração de logs,
   métricas, tracing e telemetria. Registre versão quando disponível, mas não
   trate uma dependência declarada como instrumentação efetivamente usada.
...
2. Para cada artefato de container ou build, registre estágios, imagem base,
   projeto ou artefato produzido, diretório de trabalho, entrypoint/comando,
...
- **Fato:** conclusão diretamente observada.
  - **Confiança:** alta
  - **Evidência:** `caminho/arquivo:linha` — componente, símbolo, configuração ou teste.

- **Inferência:** conclusão derivada de evidências, com a justificativa.
  - **Confiança:** média
  - **Evidência:** `caminho/arquivo:símbolo` — por que a evidência a sustenta.
...
```markdown
# Inventário da aplicação: <nome ou módulo>
...
|---|---|---|
| Repositório/módulo | <perfil, manifesto e raiz analisada> | `<manifesto do perfil>` |
...
| Recurso | Banco/tipo | Tabela/coluna, sequência ou rotina | Assinatura/operação | Origem na aplicação | Transação/erros | Fluxo | Evidência |
|---|---|---|---|---|---|---|---|
...
| ID/lote | Operação/mecanismo | Formação/origem | Tamanho/critério | Origem na aplicação | Transação/commit | Erro/rollback/retry | Evidência |
|---|---|---|---|---|---|---|---|
...
| Tipo/coluna | Tipo no banco | Representação na aplicação | Leitura/escrita | Conversão | Fluxo exposto | Riscos/evidência |
|---|---|---|---|---|---|---|
